-- ============================================
-- CONSOLIDATED DATABASE SCRIPT FOR WEBUI
-- Single database: webui_db
-- ============================================

-- Drop webui_auth if exists (consolidating into webui_db)
-- WARNING: Backup data first before running this!
-- DROP DATABASE IF EXISTS webui_auth;

-- Create and use the single database
...
CREATE TABLE IF NOT EXISTS agent_settings (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    name VARCHAR(100) NOT NULL COMMENT 'Agent name displayed in dropdown',
...
CREATE INDEX IF NOT EXISTS idx_user_configs_user_id ON user_configurations(user_id);
CREATE INDEX IF NOT EXISTS idx_agent_settings_user_id ON agent_settings(user_id);
CREATE INDEX IF NOT EXISTS idx_agent_settings_name ON agent_settings(setting_name);
CREATE INDEX IF NOT EXISTS idx_jira_xray_user_id ON jira_xray_settings(user_id);
CREATE INDEX IF NOT EXISTS idx_jira_xray_active ON jira_xray_settings(is_active);

-- ============================================
-- ADD 'name' COLUMN TO agent_settings IF NOT EXISTS
-- (For existing databases that don't have this column)
-- ============================================
-- Run this separately if needed:
-- ALTER TABLE agent_settings ADD COLUMN name VARCHAR(100) NOT NULL DEFAULT '' AFTER user_id;
-- UPDATE agent_settings SET name = setting_name WHERE name = '';
